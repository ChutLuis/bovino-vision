...
(`test_morphometry_smoke.py`) y la campaña de calibración: `test_measure_campana.py` (distancia, niveles, corrección y selección a 3 m), `test_eval_weight_campana.py` (reproduce a = 0.375, b = 0.706 y MAPE 7.71 % del piloto; falla limpio sin pesos), `test_make_weight_bundle.py` (bundle == métricas) y `test_report_campana.py` (pendiente por distancia e ICC sobre datos sintéticos). `test_pipeline_smoke.py` es un E2E con cámara y se corre a mano.

## Datos y modelos

- `data/field/`: fotos curadas de junio de 2026, pesos y morfometría (`data/field/README_DATASETS.md`).
- `data/field/campana_20260912/`: bitácora, fotografías por animal y medidas por fotografía de la campaña de calibración del 12 de septiembre de 2026; los crudos (1226 JPG) quedan fuera del repositorio en `~/Documents/Thesis_photos_12_09/` con `MANIFEST.sha1` (`--fotos` o `$BOVINO_CAMPANA_RAW`).
